...
//Et voilŕ. Droplet done.
//Write down some basic information for your own future reference:
	Droplet Name: [Droplet name] DirectDataDeposition
	IP Address: [IP address] 188.166.32.157
	Username: root (it's always "root")
	Password: [Droplet password] //Your droplet's password will also be mailed to you.
...
//[Sometimes Ubuntu wants to be updated. I believe the final version of this guide should include a few side tips on how to behave when this occurs.
//If you happen to update Ubuntu, please consider writing down a paragraph on how to do it].
...
//Open Putty
	Find Droplet ipv4 in DigitalOcean Droplet page and copy it
	Open PuTTY > Session and paste the ipv4 in the "Host Name (or IP adress)" line
	Write a session name in the "Saved Sessions" line
	Save 
	Select saved session and Open it
	Just reply "yes" in response to PuTTY's Security Alert
...
//That will copy the selected text. In order to paste text in PuTTY, instead, press the right button of your mouse. //Ctrl-C works just fine

//Now that you are in PuTTY:
...
//This is going to be the folder for our scripts
...
Console:
	...
	Choose 1-4 [2]:
User:	Choose 1-4 [2]: 2 // make sure that you select "/bin/nano" as it doesn't always appear as the second option
Console:
...
//Create new repository
	Set the repository name [I recommend Repository[Project Name]]
...
//Leave PuTTY
//Check Github repository: the README file should have appeared
//Back to PuTTY
...
User:	[since none of the listed keys end in .pub, that means no public keys are present. We'll need to create one.]
...
//Leave PuTTY
//Go to GitHub and create a new SHH key (https://github.com/settings/ssh/new)
	Pick a name for the key (I recommend "SSHkey[Project name]")
	Copy-paste the SHH key in the 'Key' box.
	Enter your GitHub password if prompted to
	SHH key has been created and can be found in https://github.com/settings/keys.
//Back to PuTTY

//Now it's time to test if this all works on the server:
...
//If you see permission denied instead, please use this guide to resolve the issue (it's probably a typeo): https://help.github.com/en/enterprise/2.16/user/articles/error-permission-denied-publickey
...
//Now comes the part of setting up the auto-sync to github
